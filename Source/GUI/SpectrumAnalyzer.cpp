...
void SpectrumAnalyzer::drawFFTAnalysis(juce::Graphics &g, juce::Rectangle<int> bounds){
    ...
    leftChannelFFTPath.applyTransform(AffineTransform().translation(responseArea.getX(), /*responseArea.getY()*/0));
    
    g.setColour(Colour(97u, 18u, 167u)); //purple-
    g.strokePath(leftChannelFFTPath, PathStrokeType(1.f));
    
    auto rightChannelFFTPath = rightPathProducer.getPath();
    rightChannelFFTPath.applyTransform(AffineTransform().translation(responseArea.getX(), /*responseArea.getY()*/0));
...
void SpectrumAnalyzer::update(const std::vector<float> &values)
{
    ...
    midBandGR = values[MidBandOut] - values[MidBandOut];
    highBandGR = values[HighBandOut] - values[HighBandOut];
